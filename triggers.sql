create function max_number_of_cars()
    ...
    $$
        ...
        begin
            select Cliente.tipo from Cliente join Veiculo on (Veiculo.nif = Cliente.nif) where (Cliente.nif = new.nif) into tipo_cliente;
            select count(NIF) numero_veiculos from Veiculo where (Veiculo.nif = new.nif) into numero_veiculos;
            for tipo_cliente in select tipo from Cliente 
                loop
                    if(tipo_cliente = 'P') then 
                        if (numero_veiculos >= 3) then 
                            return old;
                            raise notice 'Cliente não pode ter mais veículos associados';
                        else return new;
                        end if;
                    else return new;
                    end if;
